...
import com.bittrex.EncryptionUtility;
import com.bittrex.ReconnectionAttemptsExceededException;
...
public class Bittrex {
    ...
    private final String API2_VERSION = "2.0";
    ...
    public void setAuthKeysFromTextFile(String textFile) { // Add the text file containing the key & secret in the same path as the source code
        ...
        try (Scanner scan = new Scanner(getClass().getResourceAsStream(textFile))) {
            ...
            apikey = "28a82ab3aee54e139f3005518859adf0";
            secret = "f1f69f069511439782f89d2738845fee";
    ...
    public String sellLimit(String market, String quantity, String rate, String conditionType, String targetRate) { // Places a limit sell in a specific market; returns the UUID of the order

        return getJson(API_VERSION, MARKET, "selllimit", returnCorrectMap("market", market, "quantity", quantity, "rate", rate, "ConditionType", conditionType, "Target", targetRate));
    }
    ...
    public static BittrexReply getMapsFromResponse(String response) {

        final List<HashMap<String, String>> maps = new ArrayList<>();
        BittrexReply br;
        String newString;

        CharSequence charSequence = "\"success\":false";
        if (response.contains(charSequence)){
            return new BittrexReply();
        }

        if (!response.contains("[")) {
            int indexOfOpenBrace = response.indexOf("{", 1);
            int lastIndexOfCloseBrace = response.lastIndexOf("}");
            newString = response.substring(0,indexOfOpenBrace)+"["+response.substring(indexOfOpenBrace,lastIndexOfCloseBrace)+"]"+response.substring(lastIndexOfCloseBrace);

//
//            String newStringSingle1 = response.substring(0, indexOfOpenBrace);
//            String newStringSingle2 = response.substring(indexOfOpenBrace);
//            String newStringSingle21 = newStringSingle2.substring(0, lastIndexOfCloseBrace);
//            String newStringSingle3 = newStringSingle2.substring(lastIndexOfCloseBrace+1);
//            newString = newStringSingle1 + "[" + newStringSingle21 + "]" + newStringSingle3;

//            int lastIndex = response.lastIndexOf("\"result\":");
//            int resultLength = "\"result\":".length();
//            int indexOfBrace = response.indexOf("}");
//            String subString = response.substring(lastIndex + resultLength, indexOfBrace + 1);
//            HashMap<String, String> map = jsonMapToHashMap(subString);
//            maps.add(map); // Sorry.
//            maps.add(jsonMapToHashMap(response.substring(response.lastIndexOf("\"result\":") + "\"result\":".length(), response.indexOf("}") + 1))); // Sorry.

        } else {

//            int indexOf = response.indexOf("\"result\":");
//            int resultLength = "\"result\":".length();
//            int lastIndexOf = response.lastIndexOf("]");
//            int sum = indexOf+resultLength+1;
//            final String resultArray = response.substring(sum, lastIndexOf);
//
...
//                maps.add(jsonMapToHashMap(map));
        ...
        br = jsonMapToHashMap(newString);
        return br;
    }

    private static BittrexReply jsonMapToHashMap(String jsonMap) {

        final Gson gson = new GsonBuilder().setPrettyPrinting().create();

        return gson.fromJson(jsonMap, BittrexReply.class);
    }
